...
namespace Chaos.NaCl.Ed25519Ref10
{
	...
	{
		public static void scalarmult(Span<byte> q, ReadOnlySpan<byte> n, ReadOnlySpan<byte> p)
		{
			FieldOperations.fe_frombytes2(out var p0, p);
			scalarmult(out var q0, n, in p0);
			FieldOperations.fe_tobytes(q, in q0);
		}

		internal static void scalarmult(out FieldElement q, ReadOnlySpan<byte> n, in FieldElement p)
		{
			...
			FieldOperations.fe_invert(out z2, in z2);
			FieldOperations.fe_mul(out x2, in x2, in z2);
			q = x2;
			CryptoBytes.Wipe(e);
		}
	}
