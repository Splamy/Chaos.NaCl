namespace Chaos.NaCl.Ed25519Ref10
{
	...
	{
		internal static void fe_invert(out FieldElement result, in FieldElement z)
		{
			FieldElement t0;
			FieldElement t1;
			FieldElement t2;
			...
			fe_sq(out t0, in z); //for (i = 1; i < 1; ++i) fe_sq(out t0, ref t0);
			...
			fe_sq(out t2, in t0); //for (i = 1; i < 1; ++i) fe_sq(out t2, ref t2);
